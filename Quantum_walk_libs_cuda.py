...
up = cp.array((1,0))
down = cp.array((0,1))
hadamar_coin = cp.array([[1,1],[1,-1]])*(0.5**0.5)
...
def generate_single_coin(theta):
    ...
    """
    return cp.array([[cp.cos(theta), -cp.sin(theta)], [cp.sin(theta), cp.cos(theta)]])
...
def print_dist(dist, N):
    ...
    """
    limit = N//2
    prob = cp.abs(dist[0])**2+cp.abs(dist[1])**2
    prob *= 100
    plt.plot(cp.arange(-limit,limit+1), prob)
...
def coin_flip(coin,dist):
    ...
    dist[:,:] = cp.matmul(coin,dist)

def old_coin_flip(dist, coin, N):
    for i in range(N):
        dist[:,i] = cp.matmul(coin,dist[:,i])
...
def generate_coin_array_random(n):
    ...
    thetas = cp.random.uniform(cp.deg2rad(30), cp.deg2rad(45), n)
    
    # Gerando as matrizes das moedas quânticas
    coin_array = cp.array([generate_single_coin(theta) for theta in thetas])
    
    return coin_array


def print_dists(dists_cp, N, r):
    """
    ...
    print("--- %s seconds to convert ---" % (time.time() - start_conv_time))
    start_graph_time = time.time()
    for dist in dists:
        prob = np.abs(dist[0])**2+np.abs(dist[1])**2
        prob *= 100/r
    #prob = cp.asnumpy(prob_cp)
...
def multi_coin_flip(coin_array, dists):
    ...
    dists[:, :, :] = cp.einsum('rij,rjk->rik', coin_array, dists)
...
def multi_coin_flip_small(coin_array, dists):
    ...
    for repetition in range(coin_array.shape[0]):
        dists[repetition] = cp.dot(coin_array[repetition], dists[repetition])
